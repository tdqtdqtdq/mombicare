...
export default function Home() {
  ...
  const signatureServices = [
    { name: "Gội đầu dưỡng sinh", time: "70 phút", image: "/img/goi-dau.jpg", desc: "Làm sạch sâu kết hợp massage ấn huyệt vùng đầu, cổ vai gáy, đả thông kinh lạc, mang lại giấc ngủ ngon." },
    { name: "Massage vai cổ gáy trị liệu", time: "45 phút", image: "/img/co-vai-gay-tri-lieu.jpg", desc: "Trị liệu chuyên sâu, gỡ các điểm bó cơ, giảm nhanh tình trạng căng cơ, đau nhức hiệu quả." },
    { name: "Cấy HA căng bóng da", time: "90 phút", image: "/img/cay-ha.jpg", desc: "Cung cấp độ ẩm sâu, giúp làn da căng bóng, mịn màng và giảm thiểu nếp nhăn, mang lại vẻ tươi trẻ." },
    { name: "Lấy nhân mụn chuẩn Y khoa", time: "60 phút", image: "/img/lay-nhan-mun.jpg", desc: "Xử lý các nhân mụn chuẩn y khoa, dụng cụ tiệt trùng 100%, an toàn, hạn chế tối đa để lại thâm sẹo." },
  ];
  ...
  return (
    <main className={`min-h-screen relative text-[#2d3d25] ${beVietnam.className}`}>
        ...
        <header className="absolute top-0 w-full z-50 flex justify-between items-center px-4 py-4 md:px-16 md:py-6 text-[11px] md:text-[13px] uppercase tracking-[0.15em] font-medium border-b border-white/10">
          ...
          <nav className="hidden xl:flex gap-8 justify-center items-center">
            ...
            <div className="relative py-2" onMouseEnter={() => setIsDropdownOpen(true)} onMouseLeave={() => setIsDropdownOpen(false)}>
              <span className={`cursor-pointer transition-colors pb-1 ${isDropdownOpen ? 'text-[#8bb96e]' : 'hover:text-[#8bb96e]'}`}>Dịch vụ</span>
      ...
      {/* ================= 1. DỊCH VỤ NỔI BẬT (ĐƯA LÊN TRÊN & THU GỌN) ================= */}
      <section className="py-12 md:py-16 px-4 md:px-12 lg:px-24 bg-[#f6f9f2] text-center">
        ...
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8 md:mb-10 text-left max-w-7xl mx-auto">
          {signatureServices.map((service, index) => (
            <div key={index} className="group cursor-pointer bg-white shadow-sm hover:shadow-xl hover:shadow-[#8bb96e]/10 transition-all duration-300 rounded-2xl md:rounded-3xl overflow-hidden flex flex-col h-full border border-transparent hover:border-[#e4edd9]">
              <ScrollAnimation delay={index * 0.1} >
                {/* Thu gọn chiều cao ảnh từ 220px xuống 160px/180px */}
                <div className="relative w-full h-[160px] md:h-[180px] overflow-hidden bg-[#e4edd9]">
                  <Image src={service.image} alt={service.name} fill className="object-cover" />
                </div>
                <div className="p-5 md:p-6 flex flex-col flex-1">
                  <p className="text-[10px] tracking-widest text-[#8bb96e] mb-2 uppercase font-medium">{service.time}</p>
                  <h4 className={`text-lg text-[#2d3d25] mb-2 leading-snug ${playfair.className}`}>{service.name}</h4>
      ...
      {/* ================= 2. VỀ MOMBI (THIẾT KẾ LẠI BỐ CỤC NGANG ĐỂ TIẾT KIỆM CHIỀU CAO) ================= */}
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="text-left relative z-10 order-2 lg:order-1">
            <ScrollAnimation>
              ...
              {/* Thu gọn Icons thành lưới Grid nhỏ gọn bên dưới đoạn text */}
          ...
          {/* Cột Phải: Lưới ảnh rút gọn chiều cao */}
          <div className="relative z-10 order-1 lg:order-2">
            <ScrollAnimation delay={0.2}>
              {/* Chiều cao tổng chỉ còn 350px/400px thay vì 600px */}
              ...
              <div key={index} className="snap-center shrink-0 w-[280px] md:w-[350px]">
                <ScrollAnimation delay={index * 0.1}>
                  <div className="bg-white/80 backdrop-blur-md p-6 md:p-8 rounded-3xl text-left border border-[#e4edd9] shadow-sm hover:shadow-xl hover:shadow-[#8bb96e]/10 transition-all duration-300 flex flex-col h-full group/card">
                    ...
                    <p className="text-[#5c6e51] text-sm leading-relaxed italic mb-6 flex-1 group-hover/card:text-[#2d3d25] transition-colors">"{item.quote}"</p>
      ...
      <section className="py-12 md:py-16 px-4 md:px-12 lg:px-24 bg-white border-t border-[#e4edd9]">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
          ...
          <ScrollAnimation delay={0.2}>
            {/* Thu gọn chiều cao MAP xuống 300px/350px */}
